...
function getPicturesForUser(req, res) {
  pictureModel
    .getPicturesForUser(req.params.id)
    .then((pictures) =>
      res.render("userGallery", { pictures, ID: req.cookies.ID }),
    )
...
function getPicture(req, res) {
  pictureModel
    .getPicture(req.params.pid)
    .then((picture) => {
      let currentUserID = parseInt(req.cookies.ID);
      if (picture.u_id === currentUserID)
        res.render("picture", { picture, ID: req.cookies.ID });
